	...
	public MyDBFaultTolerantServerZK(NodeConfig<String> nodeConfig, String myID, InetSocketAddress isaDB) throws IOException {
		...
		try {
			...
			if (!myNodeConfig.containsNodeID(myID)) {
				...
				myNodeConfig.saveToFile(myNodeConfig);
		...
		new Thread(() -> {
			try {
				Thread.sleep(3000); // Delay for a predefined period
				myNodeConfig.loadFromFile();
...
//			}
	...
	private void watchServerNode(String serverID) {
		String serverNodePath = "/servers/" + serverID.trim(); // Trim here as a precaution
		try {
			zookeeper.exists(serverNodePath, event -> {
				...
				// Optionally re-register the watch
			});
		} catch (Exception e) {
	...
	protected void handleMessageFromClient(byte[] bytes, NIOHeader header) {
		System.out.println("Client Message: " + myID + " received the message and forwarding to leader: " + leader );
		...
		try {
			...
			request = json.getString(MyDBFaultTolerantServerZK.Keys.REQUEST
					.toString());
		...
		try {
			...
			packet.put(MyDBFaultTolerantServerZK.Keys.REQUEST.toString(), request);
			packet.put(MyDBFaultTolerantServerZK.Keys.TYPE.toString(), MyDBFaultTolerantServerZK.Type.REQUEST.toString());
		...
		if(json!=null){
			try{
				json.put(MyDBFaultTolerantServerZK.Keys.RESPONSE.toString(),
						response);
...
//		System.out.println("In handleMessageFromServer" + myID + ": received the message");
		...
		try {
			String type = json.getString(MyDBFaultTolerantServerZK.Keys.TYPE.toString());
			if (type.equals(MyDBFaultTolerantServerZK.Type.REQUEST.toString())) {
				if (myID.equals(leader)) {

					// put the request into the queue
					Long reqId = incrReqNum();
					json.put(MyDBFaultTolerantServerZK.Keys.REQNUM.toString(), reqId);
					...
					if (isReadyToSend(expected)) {
						...
						if (proposal != null) {
							proposal.put(MyDBFaultTolerantServerZK.Keys.TYPE.toString(), MyDBFaultTolerantServerZK.Type.PROPOSAL.toString());
							enqueue();
				...
				String query = json.getString(MyDBFaultTolerantServerZK.Keys.REQUEST.toString());
				long reqId = json.getLong(MyDBFaultTolerantServerZK.Keys.REQNUM.toString());

				session.execute(query);

				JSONObject response = new JSONObject().put(MyDBFaultTolerantServerZK.Keys.RESPONSE.toString(), this.myID)
						.put(MyDBFaultTolerantServerZK.Keys.REQNUM.toString(), reqId)
						.put(MyDBFaultTolerantServerZK.Keys.TYPE.toString(), MyDBFaultTolerantServerZK.Type.ACKNOWLEDGEMENT.toString());
				...
				if (myID.equals(leader)) {
					...
					String node = json.getString(MyDBFaultTolerantServerZK.Keys.RESPONSE.toString());
					if (dequeue(node)) {
